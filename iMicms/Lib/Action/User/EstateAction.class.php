...
class EstateAction extends UserAction{
	...
	public function index(){
        ...
        $panoramic = M('Panoramic')->where($where)->field('id as pid,name,keyword')->select();
        $this->assign('panoramic',$panoramic);
    ...
    public function housetype_add(){
        ...
        $this->assign('son_data',$son_data);
        $panoramic = M('Panoramic')->where(array('token'=>session('token')))->field('id as pid,name,keyword')->select();
        $this->assign('panoramic',$panoramic);
        if($check != null){
