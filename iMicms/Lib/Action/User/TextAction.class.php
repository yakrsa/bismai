...
class TextAction extends UserAction{
	...
	public function del(){
		$where['id']= $id = $this->_get('id','intval');
		$where['uid']=session('uid');
	...
	$where['uid']=session('uid');
	$where['token']= $token = session('token');
	$res=D('Text')->field('keyword,text')->where($where)->select();
	
	$myarr[] = array('keyword'=>"关键词",'text'=>"回答");
	...
	foreach($res as $key=>$val){ 
	       $myarr[] =  array('keyword'=>$val['keyword'],'text'=>$val['text']); 
		}
	 
	 foreach($myarr as $k=>$v){
         $ks=$k+1;	 
	 $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A".$ks, $v[keyword])->setCellValue("B".$ks, $v[text]);
		
         }
         $Simple = $token."文字回复"; 
         $objPHPExcel->getActiveSheet()->setTitle($Simple);
         $filename = "Text-".$token.time().".xls";
