...
class GoldeggAction extends UserAction
{
    public function index()
    {
        ...
        $list = M('Goldegg')->field('id,title,joinnum,click,keyword,startdate,enddate,status')->where(array(
            'token' => session('token')
        ))->select();
        foreach ($list as $key => $val) {
            $list[$key]['joinnum'] = M('Goldegg_record')->where(array(
                'token' => session('token'),
                'lid' => $val['id']
            ))->count();
        }
        $this->assign('count', M('Goldegg')->where(array(
            'token' => session('token')
        ))->count());
        ...
        if (IS_POST) {
            ...
            if ($data->create() != false) {
                if ($id = $data->add()) {
                    ...
                    $this->success('活动创建成功', U('Goldegg/index', array(
                        'token' => session('token')
                    )));
                } else {
