<?php
class MembercardAction extends UserAction{
	//会员卡配置
	public function index(){
		...
		if(IS_POST){
			...
			if($data==false){
				$data=M('Member_card_set')->where(array('token'=>'sjtftx'))->find();
			}
	...
	public function create_add(){ 
		...
		if(IS_POST){
			...
			if($back!=false){
				$this->success('恭喜您共开了'.$num.'张会员卡',U('Member_card/create'));
			}else{
				$this->error('服务器繁忙请稍后再试');
			}
		}else{
	...
	public function contact_edit(){
        ...
        public function contact_del(){
			...
			$data=M('Member_card_contact')->where(array('token'=>session('token'),'id'=>$this->_get('id')))->delete();
			if($data==false){
				$this->error('服务器繁忙请稍后再试');
			}else{
				$this->success('操作成功',U('Member_card/info',array('token'=>session('token'))));
