...
class CarAction extends BaseAction{
    ...
    public function index(){
		...
		$car=M('Carset')->where(array('token'=>$token))->order('id desc')->find();
		$res=M('Reservation')->where(array('token'=>$token,'addtype' => 'drive'))->order('id desc')->find();
		$reser=M('Reservation')->where(array('token'=>$token,'addtype' => 'maintain'))->order('id desc')->find();
        $this->assign('car',$car);
		$this->assign('res',$res);
        ...
        //$carset = M('Carset')->where(array('token'=>session('token')))->order('id desc')->find();
        //$this -> assign('carset', $carset);
        $this -> display();
