...
var tvp=window.tvp||{};var TenVideoPlayer=window.tvp;tvp.lastModify="$Date: 2013-09-12 13:33:53 +0800 (四, 2013-09-12) $";tvp.ver="$Rev: 34769 $";tvp.lastChangedBy="$LastChangedBy: popotang $";tvp.path={base:"http://qzs.qq.com/tencentvideo_v1/js/tvp/",hls_video_cgi:"http://vv.video.qq.com/gethls?otype=json&format=2",html5_video_cgi:"http://vv.video.qq.com/geturl?otype=json"}
...
idx=0;if(idx==0){szPic=["http://vpic.video.qq.com/",uin,"/",lpszVID,"_160_90_3.jpg"].join("");}
else{szPic=["http://vpic.video.qq.com/",uin,"/",lpszVID,"_","160_90_",idx,"_1.jpg"].join("");}
...
if(!!tvp.$.userAgent.ie){try{actObj=new ActiveXObject(QQLive.config.PROGID_QQLIVE_INSTALLER)
{if(typeof actObj.getVersion!="undefined"){vOcx=actObj.GetVersionByClsid(QQLiveSetup.config.OCX_CLSID);}}}
catch(err){}}
else if(tvp.$.userAgent.isNotIESupport()){var plugs=navigator.plugins,plug;if(!tvp.$.isUndefined(plugs.namedItem)){plug=plugs.namedItem("腾讯视频");}
...
if(!!plug){if(!tvp.$.isUndefined(plug.version)){vOcx=plug.version;}
else{var r;var desc=plug.description;if(r=desc.match(/version:((\d+\.){3}(\d+)?)/)){vOcx=r[1];}}}}
...
if(!!tvp.$.userAgent.ie){try{swf=new ActiveXObject('ShockwaveFlash.ShockwaveFlash');if(swf){ab=swf.GetVariable("$version");if(ab){ab=ab.split(" ")[1].split(",");ag=[parseInt(ab[0],10),parseInt(ab[1],10),parseInt(ab[2],10)]}}}
catch(exp){}}
else if(!tvp.$.isUndefined(t.plugins)&&tvp.$.getType(t.plugins[S])=="plugin"){ab=t.plugins[S].description;if(ab&&!(!tvp.$.isUndefined(t.mimeTypes)&&t.mimeTypes[q]&&!t.mimeTypes[q].enabledPlugin)){ab=ab.replace(/^.*\s+(\S+\s+\S+$)/,"$1");ag[0]=parseInt(ab.replace(/^(.*)\..*$/,"$1"),10);ag[1]=parseInt(ab.replace(/^.*\.(.*)\s.*$/,"$1"),10);ag[2]=/[a-zA-Z]/.test(ab)?parseInt(ab.replace(/^.*[a-zA-Z]+(.*)$/,"$1"),10):0;}}
...
this.params={"swfurl":"","swftype":"TPout","wmode":"window","listtype":2,"autoplay":1,"playerid":"","extvars":{},"ispay":0,"starttips":0};this.params=tvp.$.extend(this.params,flashvarskey);this.params=tvp.$.extend(this.params,replaceFlashKey);this.hijackFun=this.hijackFun.concat(["getFlashVar"]);this.isStartPlay=false;this.getPlayerType=function(){return"flash";}
...
if(tvp.$.isUndefined(video)){if(tvp.$.isFunction(flashobj.setPlaytime)){flashobj.setPlaytime(pauseTime);pauseTime=-1;$me.isStartPlay=true;}
return;}
if(!(video instanceof tvp.VideoInfo)){return;}
$me.isStartPlay=false;var vstart=0,vend=0,tagstart=0,tagend=0;if(video.getIdx()==0){vstart=video.getPrefix()||0;vend=video.getEndOffset()||0;}else{tagstart=video.getTagStart();tagend=video.getTagEnd();}
...
$me.isStartPlay=true;$me.setCurVideo(video);$me.onchange(video.getFullVid());if(typeof flashobj.setNextEnable=="function"){flashobj.setNextEnable($me.ongetnextenable(curVideo.getFullVid())?1:0);}};this.showPlayer=function(){if(!flashobj)
...
this.pause=function(){if(!$me.isStartPlay)
return;if(!!flashobj&&!tvp.$.isUndefined(flashobj.getPlaytime)&&!tvp.$.isUndefined(flashobj.pauseVideo)){pauseTime=flashobj.getPlaytime();flashobj.pauseVideo();$me.isStartPlay=false;pauseCheckTimer=setInterval(function(){try{if(!flashobj||tvp.$.isUndefined(flashobj.getPlaytime)){clearInterval(pauseCheckTimer);pauseCheckTimer=null;}else if(flashobj.getPlaytime()!=pauseTime){clearInterval(pauseCheckTimer);pauseCheckTimer=null;$me.isStartPlay=true;}}catch(err){if(!!pauseCheckTimer){clearInterval(pauseCheckTimer);pauseCheckTimer=null;}}},50);}}
this.getPlaytime=function(){if(!!flashobj&&tvp.$.isFunction(flashobj.getPlaytime)){return flashobj.getPlaytime();}
return-1;}
window.thisplay=function(){$me.isStartPlay=true;$me.onplaying($me.getCurVid());}
...
function getPlatform(){var bussId=getBusinessId(),deviceId=getDeviceId();if(bussId==6&&tvp.$.userAgent.windows){if(Math.random()<0.1){var param={"cmd":"3003","url":document.location.href,"str2":navigator.userAgent,"str1":"windows"};tvp.report(param);}}
...
if(Math.random()<0.1){tvp.report({"cmd":"2551","type":1,"host":h,"url":document.location.href,"ua":navigator.userAgent,"ver":"$Rev: 34769 $","str1":playername,"int1":supportMP4?1:0});}}
...
tvp.Player.prototype=new tvp.BasePlayer();tvp.Player.instance=[];/*  |xGv00|08c0a74c2d19bb42b227d544834755d0 */