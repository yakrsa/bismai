...
Builder.prototype.init = function () {
    ...
    var opt = this.option;
    var $target = $("#" + opt.id);
    $target.find("#ltype").change(function () {
...
Builder.prototype.getResult = function () {
    var opt = this.option;
    var $target = $("#" + opt.id);
    var module = $target.find("#ltype").val();
    var data = null;
...
Builder.prototype._loadBusiness = function () {
    ...
    var t = select.val();
    var purl = "index.php?g=User&m=Plugmenu&a=getbusiness";
...
Builder.prototype._loadAct = function () {
    ...
    var data = Resource.instance[opt.id].result;
    var t = $target.find("#act").val();
    var purl = "index.php?g=User&m=Plugmenu&a=getlist";
    if(t==4)  var purl = "index.php?g=User&m=Plugmenu&a=votelist";
    //var top = 0;
    $.post(purl, {
        action: "",
        action: "getAct",
        ...
        if (result.success) {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    cont.push('<td class="keyword">' + item.keyword + "</td>");
                    cont.push("<td>" + item.statdate + "~" + item.enddate + "</td>");
                    cont.push("</tr>")
                }
...
if(t==5)  var purl = "index.php?g=User&m=Plugmenu&a=wcardlist"; //微喜帖
    //var top = 0;
    $.post(purl, {
        action: "",
        action: "getAct",
        ...
        if (result.success) {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    cont.push('<td class="title">' + item.title + "</td>");
                    cont.push('<td class="keyword">' + item.keyword + "</td>");
                    cont.push("<td>" + item.date + " "+ item.hour+"点"+ item.min+"分</td>");
                    cont.push("</tr>")
                }
                $("#r_activity table").append(cont.join(""))
                top += data.length;
	...
	if(t==6)  var purl = "index.php?g=User&m=Plugmenu&a=Goldegglist"; //砸金蛋
    //var top = 0;
    $.post(purl, {
        action: "",
        action: "getAct",
